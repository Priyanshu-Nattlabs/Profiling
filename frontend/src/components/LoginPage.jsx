...
const LoginPage = ({ initialError }) => {
  ...
  // Update error if initialError changes (e.g., from OAuth failure)
  useEffect(() => {
    if (initialError) {
  ...
  const handleGoogleLogin = () => {
    ...
    // After successful OAuth, backend will redirect to /api/auth/google/callback
    // Then we need to redirect to frontend with token
    window.location.href = `${API_BASE_URL}/oauth2/authorization/google`;
  };

  // Note: OAuth callback handling is now done in App.jsx
  // This component just displays the login form
